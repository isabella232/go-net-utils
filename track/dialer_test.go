...
func TestDialerReadWriteTracking(t *testing.T) {
	...
	gc.Convey("Dialer should properly track all bytes transferred at the application layer with resets", t, func() {
		...
		for _, payload := range []string{
			...
			for i := 0; i < len(payloadBytes); i += 2 {
				...
				bytesRead, bytesWritten := dialer.BytesReadWrittenReset()
			...
			read, written := dialer.BytesReadWrittenReset()
			gc.So(read, gc.ShouldEqual, 0)
			gc.So(written, gc.ShouldEqual, 0)

			gc.So(totalRead, gc.ShouldEqual, knownRead)
			gc.So(totalWritten, gc.ShouldEqual, knownWritten)
		}
	})
}
...
func TestDialerGetReadWriteAndClose(t *testing.T) {
	...
	gc.Convey("Dialer should allow getting reads and writes while a connection is closing with resets", t, func() {
